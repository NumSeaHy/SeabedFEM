...
# uincident(x) = exact_quadratic_pml(x, t_P, k_F(ω), k_P(ω), d_PML, σ_0, A_F, B_F, A_P, B_P, A_PML, B_PML)
u_incident(x) = exact_solution_som(x, ρ_F(ω), c_F(ω), k_F(ω), ρ_P(ω), c_P(ω), k_P(ω), P_0, t_F, t_P, d_PML, σ_0)
aux(x) = -u_incident(x)
aux2(x) = -((Jinv(x) * H(x)) ⋅ u_incident(x))
# uD_transducer = VectorValue(0.0, 1.0)
U = TrialFESpace(V, [uD_null, aux2, aux])
