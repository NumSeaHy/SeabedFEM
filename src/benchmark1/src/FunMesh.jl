...
function MeshGenerator(c_F, c_P, f, L, H, d, name)
    ...
    gmsh.model.geo.addPoint(L, H, 0, h1, 3)
    gmsh.model.geo.addPoint(0, H, 0, h1, 4)
    gmsh.model.geo.addPoint(L, d, 0, min(h1, h2), 5)
    gmsh.model.geo.addPoint(0, d, 0, min(h1, h2), 6)
    ...
    model = GmshDiscreteModel(name * ".msh")
